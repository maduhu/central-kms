...
class WebSocketService @Inject()(keyService: KeyService, batchService: BatchService) extends InputConverter with OutputConverter {
  ...
  def sidecarFlow(): Flow[Message, Message, NotUsed] = {
    val sidecarActor = system.actorOf(SidecarActor.props(keyService, batchService))
